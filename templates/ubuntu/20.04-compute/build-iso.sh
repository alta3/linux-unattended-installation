...
# get parameters
TARGET_ISO=${1:-"`pwd`/ubuntu-20.04-lui-amd64.iso"}
...
cp -R "/home/ubuntu/lui/ubuntu/20.04-compute/custom/."  "./custom"
...
cd "$TMP_DISC_DIR"
rm -r '[BOOT]'
"$BIN_XORRISO" -as mkisofs -r -V "ubuntu_2004_netboot_unattended" -J -b isolinux.bin -c boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -input-charset utf-8 -isohybrid-mbr "/usr/lib/ISOLINUX/isohdpfx.bin" -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat -o "$TARGET_ISO" ./
